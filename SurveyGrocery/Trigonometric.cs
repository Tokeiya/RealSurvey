...
//#define *(1+(int*)&x) *(1+(int*)&x)
//#define *(int*)&x *(int*)&x

namespace SurveyGrocery
{
	public static unsafe class Trigonometric
	{
			...
			unchecked
			{
				...
				if (j0 < 20)
				{
					...
					else
					{
						...
						if (((i0 & i) |(uint) i1) == 0) return x; /* x is integral */
						if (huge + x > 0.0)
						{
							/* raise inexact flag */
		...
		{
			*(1 + (int*) &x) = (*(1 + (int*) &x) & 0x7fffffff) | (int) (*(1 + (int*) &y) & 0x80000000);
			return x;
		}
		...
		{
			const double two54 = 1.80143985094819840000e+16; /* 0x43500000, 0x00000000 */
			const double twom54 = 5.55111512312578270212e-17; /* 0x3C900000, 0x00000000 */
			...
			{
				*(1 + (int*) &x) = (int) (hx & 0x800fffff) | (k << 20);
				return x;
			}
		...
		private static readonly int[] init_jk =
		{
			2, 3, 4, 6
		};
		...
		static int __kernel_rem_pio2(Span<double> x, Span<double> y, int e0, int nx, int prec, Span<int> ipio2)
		{
			const double zero = 0.0;
			const double one = 1.0;
			const double two24 = 1.67772160000000000000e+07; /* 0x41700000, 0x00000000 */
			const double twon24 = 5.96046447753906250000e-08; /* 0x3E700000, 0x00000000 */
		...
		static int __ieee754_rem_pio2(double x, Span<double> y)
		{
			//#define __HI(x) *(1+(int*)&x)
			int hi(double d) => *(1 + (int*) &d);

			const double zero = 0.00000000000000000000e+00;  /* 0x00000000; 0x00000000 */
...
/* pio4lo */ 3.06161699786838301793e-17 /* 3C81A626, 33145C07 */
		...
		static double __kernel_tan(double x, double y, int iy)
		{
			 ...
			 * x^5(x^2*(xxx[2]+x^4*xxx[4]+...+x^22*[xxx12]))
			 */
		...
		static double __kernel_cos(double x, double y)
		{
			...
			double a, hz, z, r, qx;
			int ix;
			ix = *(1+(int*)&x) & 0x7fffffff;  /* ix = |x|'s high word*/
			if (ix < 0x3e400000)
			{           /* if x < 2**27 */
		...
		static double __kernel_sin(double x, double y, int iy)
		{
			...
			double z, r, v;
			int ix;
			ix = *(1+(int*)&x) & 0x7fffffff; /* high word of x */
		...
		public static double ToRadians(double deg)
		{
			...
			if (double.IsInfinity(deg) || deg == 0.0)
			{
				return deg;
			}
		...
		public static double ToDegrees(double rad)
		{
			...
			if (double.IsInfinity(rad) || rad == 0.0)
			{
				return rad;
			}
